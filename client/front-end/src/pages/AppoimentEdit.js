...
const AppointmentsEdit = ({ appointment = {}, message, goods = [], checkLines = [], canCancel }) => {
    ...
    <div className='container'>
      <div className='col-12'>
        ...
        <form onSubmit={handleSubmit}>
          <div className='row'>
            <div className='row col-12 justify-content-between'>
              <div className='col-3'>
                ...
                <label htmlFor='active' className='form-label'>Активная</label>
              </div>
              ...
              <h3
                className='mb-0'>{true ? 'Пол пациента: Мужской' : 'Пол пациента: Женский'}</h3>
            </div>
          </div>
          <div className='row col-12 justify-content-between'>
            <div className='row col-4'>
              <h4>Заключение</h4>
              <textarea rows='15' value={appointment.conclusion} disabled={readOnly} name='conclusion'
                  ...
                  {readOnly ? (
                    ...
                    <>
                      <tr>
                        ...
                        <td><input type='number' disabled step='0.01' name='price' className='form-control price'
                                   onChange={handleInputChange} /></td>
                        <td><input type='number' step='1' name='qty' className='form-control qty'
                                   onChange={handleInputChange} /></td>
                        <td><input type='number' disabled step='0.01' name='total' className='form-control total'
                                   onChange={handleInputChange} /></td>
                      </tr>
                      {checkLines.map((checkLine) => (
                        <tr key={checkLine.id}>
                          ...
                          <td><input type='number' disabled step='0.01' name='price' className='form-control price'
                                     value={checkLine.price} /></td>
                          <td><input type='number' disabled step='1' name='qty' className='form-control qty'
                                     value={checkLine.qty} /></td>
                          <td><input type='number' disabled step='0.01' name='total' className='form-control total'
                                     value={checkLine.price * checkLine.qty} /></td>
                        </tr>
                      ))}
                    </>
                  )}
          ...
          {readOnly ? null : (
            <button className='w-100 my-3 btn btn-primary btn-lg' type='submit'>Сохранить</button>
          )}
          {canCancel && (
